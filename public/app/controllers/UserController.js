...
.controller("UserController", function (CollectService, $location, $localStorage){
    ...
    userCtrl.connect = function (pseudo, mdp){
    	CollectService.connect(pseudo, mdp).then(function (result){
            if(result.data[0].success){
                $localStorage.collectionite_id_user = result.data[0].id_user;
                $localStorage.collectionite_username = result.data[0].user;
                $location.path("/home");
            }
                    ...
                    if(isEmail(email)){
                        findUserOrEmail("email", email).then(function (result){
                            if(result.data[0].success){
                                ...
                                CollectService.createUser(pseudo, mdp, nom, prenom, email).then(function (result){});
                                userCtrl.showNewUserForm = false;
    ...
    function redirectIfUser(){
        if($localStorage.collectionite_id_user != undefined) $location.path("/home");
    };
    ...
    function findUserOrEmail(type, userOrEmail){
        return CollectService.findUserOrEmail(type, userOrEmail);
    }
